...
The [[https://github.com/jphmrst/cl-nst][nst]] test framework is used.
...
this function is the start point to run all tests and return true if all test cases pass.
#+BEGIN_SRC lisp :tangle test
(defun run-test ()
...
To run all tests in demo project ~literate-demo~, please load it by yourself, it'll be loaded by [[https://travis-ci.com/jingtaozf/literate-lisp][travis ci]] which will
load config file [[./.travis.yml]] automatically every time there is a new git change.
