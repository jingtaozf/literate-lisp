...
literate programming can be simple in one org file containing both documentation and source codes,
...
some for demo, and some for test, so a new org code block option ~tangle~ to decide to
...
Firstly a new source file class for org files define in ASDF package.
#+BEGIN_SRC lisp
(in-package :asdf)
...
(defmethod asdf:perform :around (o (c asdf:org))
  (let ((*readtable* (ensure-readtable ':org)))
    ...
    (call-next-method)))
#+END_SRC
Then after this package  load, one org file can load by ASDF automatically.
* Test cases
** Preparation
