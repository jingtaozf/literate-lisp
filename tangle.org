...
(defmacro with-literate-syntax (&body body)
  `(let ((*readtable* *org-readtable*))
     ,@body))
#+END_SRC
In above macro, we add the org read table in slime environment,
...
#+BEGIN_SRC lisp :tangle test
(nst:def-test-group read-org-code-block-header-arguments ()
  (nst:def-test t1
      (:equal nil) (read-org-code-block-header-arguments "" 0))
  (nst:def-test t2
    (:equal '(:tangle :no)) (read-org-code-block-header-arguments " :tangle no  " 0))
  (nst:def-test t3
    (:equal '(:tangle :no)) (read-org-code-block-header-arguments " :tangle no" 0)))
#+END_SRC
...
(defun run-test ()
  ...
  (multiple-value-bind (status checks passed error fail warn)
    ...
    (format t "~&nst test status for literate-lisp:~a, checks:~d, passed:~d, error:~D,fail:~D,warn:~D~%"
