...
*** sly
For [[https://github.com/joaotavora/sly][sly]], it allows registering a routine to compile and load files on the Lisp side like this.
#+BEGIN_SRC lisp :load no
...
#+END_SRC
This is most effective method for incorporating literate lisp because it does not alter the original compile and load routines in Common Lisp,
thus maintaining their intended behavior.
