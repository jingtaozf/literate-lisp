...
- [[#read-org-source-file][Read Org source file]]
...
- [[#web-syntax][WEB syntax]]
  ...
  - [[#implementation][implementation]]
    - [[#the-storage-and-creation-of-code-blocks][The storage and creation of code blocks]]
    - [[#expand-form-with-web-syntax][expand form with WEB syntax]]
...
* WEB syntax
The [[https://www-cs-faculty.stanford.edu/~knuth/cweb.html][CWEB]] syntax is very flexible to organize structured code blocks and it support forward reference by a two pass scanning.

But most Common Lisp compilers are single pass, so it's hard to forward reference a code block when it's a macro,
so we will not support a forward reference for WEB syntax in Common Lisp.

In Common Lisp, we will use a native macro to reference a code block which will expand to its real code block in compiling automatically.
