...
But most Common Lisp compilers are single pass, so it's hard to forward reference a code block especially it's a macro.

Common Lisp can ba a fuctional language, if you don't use a macro, then it has been ready to be organized as structured code blocks.

Sometimes we want to expand some
