...
Allegro Lisp has extended the [[https://franz.com/support/documentation/10.0/doc/implementation.htm#reader-macros-2][syntax]] for feature test.
LispWorks has different behavior when test feature, for example it will not report error with additional argument in feature expression
(try feature expression =(not lispworks 6)=).
...
Please note that in Allegro Lisp, the standard readtable is [[https://franz.com/support/documentation/current/doc/implementation.htm#readtable-2][read-only]].
We have to update it before modifications.
#+BEGIN_SRC lisp
